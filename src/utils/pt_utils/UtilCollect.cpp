...
/*=========================================================================*\
    Function:	ProUtilBodyFindGeomitemVisitAction()
    Purpose:	Call ProUtilFindGeomitemVisitAction 
		  	    based on ProUtilGeomitemByNameFilterAction()	
    Returns:	PRO_TK_NO_ERROR for success
\*=========================================================================*/
ProError ProUtilBodyFindGeomitemVisitAction(
    ProAppData geomitem,	    /* In : The geomitem */
    ProError	status,		    /* the filter func status */
    FindByName   *p_findbyname)	    /* In : The find structure */
{
    if ( PRO_TK_NO_ERROR == ProUtilGeomitemByNameFilterAction(geomitem, p_findbyname) )
		   status = ProUtilFindGeomitemVisitAction(geomitem, status, p_findbyname);	
...
//         status = ProSolidBodiesCollect(p_solid, &bodies);
// 		TEST_CALL_REPORT("ProSolidBodiesCollect()", "ProUtilCollectSolidSurfaces()",
// 					    status, status != PRO_TK_NO_ERROR);
...
// 					ProUtilCollect3ParamOpaqueVisitAction, 
// 					( ProAppData ) &p_surfaces );
//  		      TEST_CALL_REPORT("ProSolidBodySurfaceVisit()", 
    ...
    ProGeomitem *p_geomitem)	    /* Out: found surface,user's memory */
{
    ...
    ACTION_TYPE( "ProSurfaceVisitAction", "ProSurfaceFilterAction" )
        status = ProSolidBodiesCollect(p_solid, &bodies);
		TEST_CALL_REPORT("ProSolidBodiesCollect()", "ProUtilFindSurfaceByName()",
					    status, status != PRO_TK_NO_ERROR);
		...
		if (status == PRO_TK_NO_ERROR)
  	    {
		    ...
		    // {
		    //   status = ProSolidBodySurfaceVisit((ProSolidBody*)&bodies[i], 
			// 		ProUtilBodyFindGeomitemVisitAction, 
			// 		( ProAppData ) &find );
 		    //   TEST_CALL_REPORT("ProSolidBodySurfaceVisit()", 
			// 	"ProUtilFindSurfaceByName()", status, 
			// 	status != PRO_TK_NO_ERROR && status != PRO_TK_E_NOT_FOUND);		
		    // }
		}				
