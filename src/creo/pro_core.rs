...
pub fn pro_asynchronous_main(argc: i32, argv: Vec<String>) -> i32 {
    ...
    unsafe { ffi::ProAsynchronousMain(argc, p_argv.as_ptr() as *mut *mut i8) }
}
...
pub fn pro_toolkit_main(argc: i32, argv: Vec<String>) {
    ...
    let mut p_argv: Vec<_> = cstr_argv
        ...
        .collect();
    p_argv.push(std::ptr::null());
    unsafe { ffi::ProToolkitMain(argc, p_argv.as_ptr() as *mut *mut i8) }
}
...
pub fn pro_asynchronous_event_loop() -> i32 {
    unsafe { ffi::ProAsynchronousEventLoop() }
}
...
pub fn pro_engineer_end() -> i32 {
    unsafe { ffi::ProEngineerEnd() }
}
